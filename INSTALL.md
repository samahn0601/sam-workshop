...
→ Claude가 clone(또는 ZIP)→평평 복사→목록 확인까지 직접 수행. 17 skill 폴더 + `_shared`가 보이면 성공.

### ⑤ **새 세션 1회** → 탐지 확인 → 첫 발화

`.claude/skills/`가 새로 생긴 직후라 **새 세션을 한 번 더** 연다 (skill은 세션 시작 시점에 스캔). 입력창에 `/`를 쳐서 `journal-fit-check` 등이 목록에 보이면 탐지 성공. 이어서:
```
"내 논문 어느 저널에 투고할까?"
```
...
⚠️ **핵심: 우산 폴더 금지.** `skills/sam-workshop` 폴더째 복사하면 2단계 깊이가 되어 **17개 전부 미탐지**된다(실측 확인된 함정). 반드시 **내용물**을 `.claude/skills/` 바로 아래로.
...
ls ~/papers/my_paper_2026/.claude/skills/   # → 17 skill 폴더 + _shared 가 바로 보여야 함
```
...
dir "$env:USERPROFILE\papers\my_paper_2026\.claude\skills"   # → 17 skill 폴더 + _shared
```
...
```
3. 우산 폴더가 있으면: 그 안의 17개 폴더 + `_shared`를 한 단계 위(`.claude/skills/`)로 이동 후 새 세션.
