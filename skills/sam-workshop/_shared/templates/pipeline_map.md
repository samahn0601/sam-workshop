...
>
> "AI에게 통제권을 넘기는" 장치가 아니라, **저자가 자동화의 양을 단계별로 골라 쓰는** 장치다. (3AI consult 2026-05-31 합의 반영: 2 floor + ①⑦ soft floor + "정지 없이 진행" 용어 + 현장 2모드)
...
```
  ...
  ①Idea → ②Research → ③Story → ④Draft → ⑤Verify → ⑥Critic → ⑦Package → ⑧Wrap
   ⭐✋        ▶️         ✋        ▶️        ✋          ✋        ⭐✋         ▶️

  ▶️ 정지 없이 진행 (요약 확인·저자 책임은 유지)    ✋ 정차: 저자 검토 [승인/수정/계속]
  ⭐ soft floor: 어떤 모드에서도 최소 1회 저자 확인 (① Idea · ⑦ Package)
  🔒 Floor: 감지 시 모드 무관 강제 정차 — 🔒Medical Safety / 🔒Publication Ethics
```
...
### ⭐ Soft Floor — ① Idea Lock, ⑦ Package (모든 모드 공통)
...
- **⑦ Package**: AI 활용 공개문·cover letter 핵심 항목(target journal, article type, AI disclosure 포함 여부, originality/no dual submission, COI, corresponding author)은 어떤 모드든 저자가 보고 승인.
...
> ⑦ Package의 AI disclosure는 soft floor이자 Publication Ethics floor — 어떤 모드든 저자 확인을 반드시 거친다. ⑦에서 생성한 공개문은 **"저널 지침 확인 전 임시 문안"**으로 표시(저널별 요구가 빠르게 변동).
...
제출 전 체크박스: ☐ 모든 주장·인용을 확인해야 함을 이해 ☐ AI는 저자가 아니며 최종 책임은 본인 ☐ 저널 AI disclosure 지침 별도 확인.
...
- 진행 중 변경: "⑥부터 정지 없이 진행" → ⑥ 이후 review 해제 (단 ⑦⭐·floor는 유지)
...
> "오늘 여러분은 이 8단계 파이프라인을 손에 넣습니다. 각 단계를 **정지 없이 흘려보낼지, 직접 운전대를 잡을지** 고릅니다 — 오늘은 🛡️표준으로. 단, ①주제·⑦윤리(AI 공개문)와 환자에게 닿는 결정(🔒)은 어떤 모드에서도 저자가 반드시 봅니다."
