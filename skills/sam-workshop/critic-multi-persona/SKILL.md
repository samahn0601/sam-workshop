...
description: >
  ...
  (Reviewer-Method, Reviewer-Stats, Editor) plus optional external GPT-5.5
  critic, integrated by an Area Chair persona. Trigger when user says "critic",
  ...
  paper_home/04_draft/manuscript.md (and post-Step-5 verify reports). Output:
  paper_home/06_critic/{claude_personas.md, gpt55_critic.md (optional),
  area_chair_round1.md}. Pipeline position: sam-workshop Step 6 / Self-Gate C.
  Medical context: ICMJE manuscript critique standards, R1-R4 reward-hacking
  defense, multi-engine cross-validation when GPT-5.5 available. Trigger
...
> Multi-engine peer review simulation. Claude 다중 페르소나 + 외부 GPT-5.5 → Area Chair 통합. 워크숍 단순화 모드 표준.

## 모드

| 모드 | 시간 | 페르소나 |
|---|---|---|
| `workshop-mini` | 35분 | Claude 3-persona + GPT-5.5 1회 (선택) → AC |
| `standard` | 60분 | + R1-R4 reward hacking full check |
...
| (선택) **External GPT-5.5** | 외부 시각 (Claude 외) | top 5 issue |
| **Area Chair** (Claude) | 통합, R1–R4 reward hacking 체크 | Must Fix / Should Fix / Optional |
...
각 페르소나 프롬프트를 Cowork에서 순차 발화. 각 5분.
...
- suggested_fix
- reward_hacking_risk (R1-R4 중 해당 시 표시)

R1 (한계 나열로 점수 올리기): "limitations" 섹션을 추가만 했는가
R2 (검증 우회): 결정적 검증 없이 LLM 의견만으로 통과
R3 (이중 카운트): 같은 이슈를 여러 곳에서 언급해서 점수 분산 회피
R4 (과장): "first/only/definitive"로 over-claim
```
...
#### Persona Editor
```
당신은 target 저널의 Editor입니다. 본 manuscript에 대해:
- 본 저널 scope 부합? 
...
### Phase 2 (10분) — 외부 GPT-5.5 critic (선택)

ChatGPT 무료/Plus에서 다음 프롬프트 발사:
...
→ 결과 복붙 → `paper_home/06_critic/gpt55_critic.md`

(GPT 무료 한도 초과 시 → Phase 3에서 Claude 4번째 페르소나 "External Skeptic"로 대체)
...
R1-R4 Reward Hacking 체크:
- R1: limitations만 추가한 척 → reject
- R2: LLM 의견만으로 통과 → reject  
- R3: 이슈 분산 → reject
- R4: over-claim → reject

Score-Driven Revert 알림:
- v2 → v3 변경 시 종합 scorecard 비교 (있으면)
- 점수 ↓ → 즉시 revert 권고
...
|---|---|---|---|---|---|
| 1 | C-Methods-3 | Methods §3.2 | 교란 변수 미보정 | R1+GPT | 다변량 분석 추가 |
| 2 | ... |
...
| `reward_hacking_reason` | string | — | reject 시 사유 (R1-R4 표시 또는 자유 텍스트) |
...
각 critic 의견 채택/거절 시:
```json
{"ts":"...","step":6,"gate":"C_verify_critic","event_type":"critique_decision","skill":"critic-multi-persona","engine":"claude-r1+gpt55","category":"methods_critique","severity":3,"description":"Multivariate adjustment missing","decision":"accepted","backlog_id":"AC-01"}
```

## 결정적 vs LLM 분리

전부 LLM. 단:
- Reward hacking 체크는 **rule-based** (R1-R4 정의 기반)
- Score comparison은 **결정적** (수치 비교)

## Solo + Claude-only fallback

GPT-5.5 무료 한도 초과 또는 ChatGPT 미사용:
...
품질은 multi-engine보다 80–85%이지만 워크숍 산출 가능.
...
2. **GPT-5.5 critic이 Claude와 너무 비슷** — 같은 학습 데이터 영향. R6 외부 grounding으로 보강
3. **Editor 페르소나가 too positive** — accept_likelihood 너무 후함. "가장 가혹한 desk editor" 페르소나 강화
4. **Score 비교 없이 진행** — Score-Driven Revert 못 잡음. v2와 v3 동시 비교 필수

## 다음 단계

→ Self-Gate C 통과 → Step 7 Figures
